...
```shell
curl -X GET http://api.discue.io/v1/listeners \
...
const response = await fetch('http://api.discue.io/v1/listeners', {
  method: 'GET',  headers
})
...
r = requests.get('http://api.discue.io/v1/listeners', headers = headers)
```
...
func main() {
  ...
  req, err := http.NewRequest("GET", "http://api.discue.io/v1/listeners", data)
  req.Header = headers
