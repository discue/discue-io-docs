...
```shell
curl -X DELETE http://api.discue.io/v1/queues/{queue_id}/listeners/{listener_id} \
...
r = requests.delete('http://api.discue.io/v1/queues/{queue_id}/listeners/{listener_id}', headers = headers)
```
...
func main() {
  ...
  req, err := http.NewRequest("DELETE", "http://api.discue.io/v1/queues/{queue_id}/listeners/{listener_id}", data)
  req.Header = headers
