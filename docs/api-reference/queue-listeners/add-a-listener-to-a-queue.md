...
api:
  ...
  name: Add a listener to a queue
...
```shell
curl -X POST http://api.discue.io/v1/queues/{queue_id}/listeners \
...
r = requests.post('http://api.discue.io/v1/queues/{queue_id}/listeners', headers = headers)
```
...
func main() {
  ...
  req, err := http.NewRequest("POST", "http://api.discue.io/v1/queues/{queue_id}/listeners", data)
  req.Header = headers
