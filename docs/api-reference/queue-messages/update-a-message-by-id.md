...
```shell
curl -X PUT http://api.discue.io/v1/queues/{queue_id}/messages/{message_id} \
...
r = requests.put('http://api.discue.io/v1/queues/{queue_id}/messages/{message_id}', headers = headers)
```
...
func main() {
  ...
  req, err := http.NewRequest("PUT", "http://api.discue.io/v1/queues/{queue_id}/messages/{message_id}", data)
  req.Header = headers
...
|---|---|---|---|
|200|[OK](https://tools.ietf.org/html/rfc7231#section-6.3.1)|Ok|[GetMessageResponse](#getmessageresponse)|
