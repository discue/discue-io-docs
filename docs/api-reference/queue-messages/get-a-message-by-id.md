...
```shell
curl -X GET http://api.discue.io/v1/queues/{queue_id}/messages/{message_id} \
...
r = requests.get('http://api.discue.io/v1/queues/{queue_id}/messages/{message_id}', headers = headers)
```
...
func main() {
  ...
  req, err := http.NewRequest("GET", "http://api.discue.io/v1/queues/{queue_id}/messages/{message_id}", data)
  req.Header = headers
